...
#ifndef genfit_RKTrackRepPt_h
#define genfit_RKTrackRepPt_h
...
/**
 * @brief AbsTrackRep with 5D track parameterization in plane coordinates: (q/pT, u', v', u, v)
 *
 * q/pT is charge over transverse momentum.
 * u' and v' are direction tangents.
 * u and v are positions on a DetPlane.
 */
class RKTrackRepPt : public RKTrackRep {

 public:

  RKTrackRepPt();
  RKTrackRepPt(int pdgCode, char propDir = 0);

  virtual ~RKTrackRepPt();

  virtual AbsTrackRep* clone() const {return new RKTrackRepPt(*this);}
 ...
 private:
  ...
  mutable std::vector<RKStepPt> RKSteps_; //! RungeKutta steps made in the last extrapolation
  mutable int RKStepsFXStart_; //!
  mutable int RKStepsFXStop_; //!
  mutable std::vector<ExtrapStepPt> ExtrapSteps_; //! steps made in Extrap during last extrapolation

  mutable TMatrixD fJacobian_; //!
  mutable TMatrixDSym fNoise_; //!

  mutable bool useCache_; //! use cached RKStepPts_ for extrapolation
